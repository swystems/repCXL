...
const TEST_MEMORY_SIZE: usize = 1024 * 1024; // 1 MiB
const TEST_CHUNK_SIZE: usize = 64;

fn setup_tmpfs_file(path: &str, size: usize) {
    let file = File::create(path).expect("Failed to create tmpfs file");
    file.set_len(size as u64).expect("Failed to set file size");
}

fn cleanup_tmpfs_file(path: &str) {
    let _ = std::fs::remove_file(path);
}

fn multi_rcxl(num: usize, node_path: &str) -> Vec<RepCXL<u64>> {
