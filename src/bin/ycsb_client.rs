// Run YCSB benchmark through the RepCXL client library. It works by importing
// a YCSB workload and executing it (rather than issuing reqs from YCSB).
//
// Input: separate load and run trace files generated by YCSB via BasicDB.
//
// Trace file format:
//   ***************** properties *****************
//   "key"="value"
...
fn parse_operations(lines: &[&str], start: usize) -> Vec<YcsbOp> {
    ...
    for &line in &lines[start..] {
        ...
        let fields = match op_type {
            OpType::Read | OpType::Scan => Vec::new(),
...
fn parse_ycsb_trace(path: &str) -> (WorkloadProperties, Vec<YcsbOp>) {
...
fn load_ycsb_workload(load_path: &str, run_path: &str) -> YcsbWorkload {
...
fn main() {
    ...
    if args.len() < 3 {
        eprintln!("Usage: {} <load_trace> <run_trace>", args[0]);
        std::process::exit(1);
    }

    let workload = load_ycsb_workload(&args[1], &args[2]);
    workload.summary();
